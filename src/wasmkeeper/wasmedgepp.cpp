...
Module::Module(std::shared_ptr<Config> config, const std::string& filepath)
    : m_config(config) {
  WasmEdge_LoaderContext* loadCxt = WasmEdge_LoaderCreate(m_config->raw());
  WasmEdge_Result res =
      WasmEdge_LoaderParseFromFile(loadCxt, &m_ctx, filepath.c_str());
...
void wasmkeeper::Vm::run() {
  ...
  res = WasmEdge_VMExecute(m_ctx, funcName, NULL, 0, NULL, 0);
  if (!WasmEdge_ResultOK(res)) {
